#ifndef INCLUDE_CORE_THREAD_POOL_HH
#define INCLUDE_CORE_THREAD_POOL_HH
...
#include "config/ThreadPoolMode.hh"
...
class ThreadPool {
    public:
        ...
        // 设置线程池cached模式下线程阈值
        void setThreadSizeThreshHold(unsigned int threshhold);
        ...
        auto submitTask(Func&& func, Args&&... args)
            ...
            if (!notFull_.wait_for(
                ...
                std::cerr << "任务队列已满，不能提交任务！" << std::endl;
                auto errTask = std::make_shared<std::packaged_task<RType()>>(
                    []() -> RType {
                        return RType();
                    });
                (*errTask)();
                return errTask->get_future();
            }
    ...
    private:
        ...
        unsigned int initThreadSize_; // 初始的线程数量
...
#endif // INCLUDE_CORE_THREAD_POOL_HH
