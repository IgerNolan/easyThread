...
// 设置线程池cached模式下线程阈值
void ThreadPool::setThreadSizeThreshHold(unsigned int threshhold) {
    if (checkRunningState()) {
        return;
    }
...
void ThreadPool::start(unsigned int initThreadSize) {
    ...
    for (unsigned int i = 0; i < initThreadSize_; i++) {
        threads_ [static_cast<int>(i)]->start(); // 需要去执行一个线程函数
                ...
                if (poolMode_ == PoolMode::MODE_CACHED) {
                    ...
                    if (std::cv_status::timeout ==
                        ...
                        if (dur.count() >= THREAD_MAX_IDLE_TIME &&
                            ...
                            // threadFunc《=》thread对象 threadid =>
                            // thread对象 => 删除
                            threads_.erase(
                                threadid); // std::this_thread::getid()
                            curThreadSize_--;
                            idleThreadSize_--;
